...
// Working of proxy design pattern in JavaScript
...
const proxiedPerson = new Proxy(person, {
  ...
  set(obj, prop, value){
    // intercept get
    console.log(`The value of the ${prop} is about to change from ${obj[prop]} to ${value}`);
    obj[prop] = value;
  }
});
...
// As JavaScript is a loosely typed language and the types are not defined, values in the objects can be updated to any type that can create bugs.

// Implementing a proxy design pattern makes sure that we have an interceptor in place that observes the changes that are being made to the original object.

// For example, if we want to add the validations to the object properties that their types cannot be changed or they should be within a certain range we can add those validations in the proxy.
...
// Even though proxies are very efficient in controlling the behavior of the object and can be used for many use cases such as intercepting, monitoring, alerting, etc.
